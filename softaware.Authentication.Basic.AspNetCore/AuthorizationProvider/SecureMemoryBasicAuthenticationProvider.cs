...
{
    private readonly Dictionary<string, SecureString> credentials;

    public SecureMemoryBasicAuthenticationProvider(IReadOnlyDictionary<string, string> credentials)
    {
        this.credentials = credentials.ToDictionary(c => c.Key, c => GetSecuredString(c.Value));
    }
    ...
    private static SecureString GetSecuredString(string value)
    {
